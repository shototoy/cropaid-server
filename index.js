...
app.patch('/api/admin/reports/:id/status', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        try {
            ...
            if (report[0]) {
                console.log('[NOTIF CREATE] Creating notification for user:', report[0].user_id, 'status:', status);
                await pool.execute(
                    `INSERT INTO notifications (user_id, type, title, message, reference_id)
                        VALUES (?, 'status_change', ?, ?, ?)`,
                    [report[0].user_id, `Report ${status}`, `Your ${report[0].type} report has been ${status}`, req.params.id.toString()]
                );
                console.log('[NOTIF CREATE] Notification created successfully');
            }
        } catch (e) {
            console.error('[NOTIF CREATE ERROR]', e);
            console.error('[NOTIF CREATE ERROR] Details:', {
                message: e.message,
                code: e.code,
...
// Get Notifications (supports polling via after_id)
...
// Mark Single Read (Supports PATCH and PUT)
...
app.patch('/api/notifications/:id/read', authenticateToken, markReadHandler);
app.put('/api/notifications/:id/read', authenticateToken, markReadHandler);

// Mark All Read (Supports PATCH and PUT)
...
app.patch('/api/notifications/read-all', authenticateToken, markAllReadHandler);
app.put('/api/notifications/mark-all-read', authenticateToken, markAllReadHandler);
...
app.post('/api/admin/pest-categories', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        console.error('Pest category creation error:', err);
...
app.put('/api/admin/pest-categories/:id', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        console.error('Pest category update error:', err);
        res.status(500).json({ error: 'Server error', details: err.message });
    }
});
...
app.post('/api/admin/crop-types', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        const [result] = await pool.execute(
            `INSERT INTO crop_types(name, variety) VALUES(?, ?)`,
            [name, variety || null]
        );
        res.status(201).json({ id: result.insertId, message: 'Crop type created' });
    } catch (err) {
        console.error('Crop type creation error:', err);
        res.status(500).json({ error: 'Server error', details: err.message });
    }
});
app.put('/api/admin/crop-types/:id', authenticateToken, requireAdmin, async (req, res) => {
    try {
        const { name, variety } = req.body;
        await pool.execute(
            `UPDATE crop_types SET name = ?, variety = ? WHERE id = ?`,
            [name, variety || null, req.params.id]
        );
        res.json({ message: 'Crop type updated' });
    } catch (err) {
        console.error('Crop type update error:', err);
        res.status(500).json({ error: 'Server error', details: err.message });
    }
});
...
// (Removed redundant duplicate notification routes)
